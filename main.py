...
class TripletNetwork2(nn.Module):
    def __init__(self):
        super(TripletNetwork2, self).__init__()
        ## encoder layers ##
        # conv layer (depth from 1 --> 16), 3x3 kernels
        hdim = 32
        bdim = 16
        self.conv1 = nn.Conv2d(3, hdim, 3, padding=1)
        self.conv2 = nn.Conv2d(hdim, hdim, 3, padding=1)
        self.conv2_1 = nn.Conv2d(hdim, hdim, 3, padding=1)
        self.conv3 = nn.Conv2d(hdim, hdim, 3, padding=1)
        self.conv3_1 = nn.Conv2d(hdim, bdim, 3, padding=1)
        # conv layer (depth from 16 --> 4), 3x3 kernels
        self.convB = nn.Conv2d(bdim, 3, 3, padding=1)
    ...
    def forward(self, x):
        ...
        l = self.actvn(self.conv2(l))
        l = self.dropout(l)
        l = self.actvn(self.conv2_1(l))
        l = self.pool(l)
        l = self.dropout(l)
        l = self.actvn(self.conv3(l))
        l = self.dropout(l)
        l = self.actvn(self.conv3_1(l))
        l = self.dropout(l)
        l = self.convB(l)
        ...
        m = self.actvn(self.conv2(m))
        m = self.dropout(m)
        m = self.actvn(self.conv2_1(m))
        m = self.pool(m)
        m = self.dropout(m)
        m = self.actvn(self.conv3(m))
        m = self.dropout(m)
        m = self.actvn(self.conv3_1(m))
        m = self.dropout(m)
        m = self.convB(m)
        ...
        r = self.actvn(self.conv2(r))
        r = self.dropout(r)
        r = self.actvn(self.conv2_1(r))
        r = self.pool(r)
        r = self.dropout(r)
        r = self.actvn(self.conv3(r))
        r = self.dropout(r)
        r = self.actvn(self.conv3_1(r))
        r = self.dropout(r)
        r = self.convB(r)
...
for epoch in range(1, n_epochs+1):
    ...
    for data in train_loader:
        ...
        if it%1000 == 0:
            #print('Saving model')
            torch.save(model.state_dict(), "/content/drive/My Drive/IML/task4/testt.pt")
    ...
    print('Saving model')
    torch.save(model.state_dict(), "/content/drive/My Drive/IML/task4/testt_epoch.pt")
